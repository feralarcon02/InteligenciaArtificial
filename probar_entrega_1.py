...
def probar_codigo(interactivo=False, saltear_errores=False):
    ...
    try:
        ...
        problemas.append(u'No se pudo encontrar el codigo python. Probablemente el nombre del archivo .py no es correcto.')
        return

    validar_tiempo(inicio, fin, 5, problemas, u'El import de la entrega demora demasiado tiempo, probablemente estan haciendo busqueda en el import. Hagan lo del if __name__ ... que se recomienda en la consigna.')

    # intentar extraer y validar la funcion resolver
    print u'Extrayendo la funcion resolver...'

    resolver = getattr(entrega_1_tradicional, 'resolver', None)

    if resolver is None:
        problemas.append(u'El modulo python no define la funcion resolver.')
        return
    ...
    if inspect.getargspec(resolver)[0] != ['metodo_busqueda', 'posicion_rey', 'controlar_estados_repetidos']:
        ...
        problemas.append(u'La funcion resolver no recibe los parametros definidos en la entrega.')
        return

    # validar el funcionamiento de la funcion resolver y el planteo del problema en general
    print u'Probando la resolucion de problemas...'
    ...
    for numero_prueba, (metodo_busqueda, posicion_rey, graph_search, limite_largo_camino, limite_tiempo) in enumerate(pruebas):
        ...
        if not interactivo or raw_input('ejecutar? (Y/n)').strip() in ('y', ''):
            try:
                ...
                validar_tiempo(inicio, fin, limite_tiempo, advertencias, u'La prueba %i demoro demasiado tiempo (mas de %i segundos), probablemente algo no esta demasiado bien.' % (numero_prueba, limite_tiempo))

                if resultado is None:
                    problemas.append(u'El resultado devuelto por la funcion resolver en la prueba %i fue None, cuando el problema tiene que encontrar solucion y se espera que retorne el nodo resultante. Puede que la funcion resolver no este devolviendo el nodo resultante, o que el problema no este encontrando solucion como deberia.' % numero_prueba)
